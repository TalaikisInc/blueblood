...
def compute_consistency_score(returns_test, preds):
    ...
    q = [sp.stats.percentileofscore(cum_preds[:, i], returns_test_cum.iloc[i], kind='weak') for i in range(len(returns_test_cum))]
...
def _plot_bayes_cone(returns_train, returns_test, preds, plot_train_len=None, ax=None):
    ...
    print(len(perc.items()))
    print(len(returns_test.index))
...
def run_model(model, returns_train, returns_test=None, bmark=None, samples=500, ppc=False, progressbar=True):
    ...
    if model == 'alpha_beta':
        model, trace = model_returns_t_alpha_beta(returns_train, bmark, samples, progressbar=progressbar)
    elif model == 't':
        model, trace = model_returns_t(returns_train, samples, progressbar=progressbar)
    elif model == 'normal':
        model, trace = model_returns_normal(returns_train, samples, progressbar=progressbar)
    elif model == 'best':
        model, trace = model_best(returns_train, returns_test, samples=samples, progressbar=progressbar)
    else:
        raise NotImplementedError('Model {} not found. Use alpha_beta, t, normal, or best.'.format(model))

    if ppc:
        ppc_samples = pm.sample_ppc(trace, samples=samples, model=model, size=len(returns_test), progressbar=progressbar)
        return trace, ppc_samples['returns']

    return trace


def plot_bayes_cone(returns_train, returns_test, ppc, plot_train_len=50, ax=None):
    """
    ...
    score = compute_consistency_score(returns_test=returns_test, preds=ppc)
    ax = _plot_bayes_cone(returns_train=returns_train, returns_test=returns_test, preds=ppc, plot_train_len=plot_train_len, ax=ax)
