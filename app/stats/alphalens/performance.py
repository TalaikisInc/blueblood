...
from pandas import DataFrame, Grouper, Series, Timedelta, DatetimeIndex, isnull, concat
from numpy import nan, sqrt, nansum
import warnings
...
def mean_information_coefficient(factor_data, group_adjust=False, by_group=False, by_time=None):
    ...
    if by_time is not None:
        grouper.append(Grouper(freq=by_time))
    if by_group:
        grouper.append('group')
...
def factor_weights(factor_data, demeaned=True, group_adjust=False, equal_weight=False):
    ...
    weights = factor_data.groupby(grouper)['factor'].apply(to_weights, demeaned, equal_weight)

    if group_adjust:
        weights = weights.groupby(level='date').apply(to_weights, False, False)

    # preserve freq, which contains trading calendar information
    # weights.index.freq = factor_data.index.freq
    return weights


def factor_returns(factor_data, demeaned=True, group_adjust=False, equal_weight=False, by_asset=False):
    """
...
def factor_alpha_beta(factor_data, returns=None, demeaned=True, group_adjust=False, equal_weight=False):
    """
    ...
    for period in returns.columns.values:
        ...
        try:
            ...
            alpha_beta.loc['Ann. alpha', period] = nan
            alpha_beta.loc['beta', period] = nan
        else:
            freq_adjust = Timedelta('252Days') / Timedelta(period)
...
def cumulative_returns(returns, period, freq=None):
    ...
    ----------
    returns: Series
        Series containing factor 'period' forward returns, the index
    ...
    Returns
    -------
    Cumulative returns series : Series
        Example:
    ...
    while len(trades_idx) > 0:
        ...
        sub_index = DatetimeIndex(sub_index, tz=full_idx.tz)
        ...
        for pret_idx in reversed(sub_index):
            ...
            def rate_of_returns(ret, period):
                return ((nansum(ret) + 1)**(1. / period)) - 1
    ...
    #
    sub_portfolios = concat(sub_returns, axis=1)
    portfolio = Series(index=sub_portfolios.index)
...
def positions(weights, period, freq=None):
    ...
    ----------
    weights: Series
        Series containing factor weights, the index contains timestamps at
    ...
    #
    portfolio_weights = DataFrame(index=weights_idx,
    ...
    for curr_time in weights_idx:
        ...
        tot_weights = concat(tot_weights, axis=1)
...
def mean_return_by_quantile(factor_data, by_date=False, by_group=False, demeaned=True, group_adjust=False):
    """
    ...
    std_error_ret = group_stats.T.xs('std', level=1).T \
        / sqrt(group_stats.T.xs('count', level=1).T)
...
def compute_mean_returns_spread(mean_returns,
    ...
    if std_err is None:
        ...
        joint_std_err = sqrt(std1**2 + std2**2)
...
def quantile_turnover(quantile_factor, quantile, period=1):
    ...
    if isinstance(period, int):
        ...
        shift = int(Timedelta(period) / freq)
...
def average_cumulative_return_by_quantile(factor_data,
    ...
    Returns
    -------
    cumulative returns and std deviation : DataFrame
    ...
    def average_cumulative_return(q_fact, demean_by):
        ...
        return DataFrame({'mean': q_returns.mean(axis=1),
    ...
    if by_group:
        ...
        return concat(returns_bygroup, axis=0)
        ...
        if group_adjust:
            ...
            q_returns = concat(all_returns, axis=1)
            q_returns = DataFrame({'mean': q_returns.mean(axis=1),
