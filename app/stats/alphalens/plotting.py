...
from numpy import asarray, nan, linspace, nanpercentile
from pandas import Series, MultiIndex, Grouper, DataFrame
...
def plot_ic_hist(ic, ax=None):
    ...
    for a, (period_num, ic) in zip(ax, ic.iteritems()):
        sns.distplot(ic.replace(nan, 0.), norm_hist=True, ax=a)
...
def plot_ic_qq(ic, theoretical_dist=stats.norm, ax=None):
    ...
    for a, (period_num, ic) in zip(ax, ic.iteritems()):
        sm.qqplot(ic.replace(nan, 0.).values, theoretical_dist, fit=True,
                  line='45', ax=a)
...
def plot_quantile_returns_bar(mean_ret_by_q, by_group=False, ylim_percentiles=None, ax=None):
    ...
    if ylim_percentiles is not None:
        ymin = (nanpercentile(mean_ret_by_q.values,
                                 ylim_percentiles[0]) * DECIMAL_TO_BPS)
        ymax = (nanpercentile(mean_ret_by_q.values,
...
def plot_quantile_returns_violin(return_by_q, ylim_percentiles=None, ax=None):
    ...
    Parameters
    ----------
    return_by_q : DataFrame - MultiIndex
    ...
    if ylim_percentiles is not None:
        ymin = (nanpercentile(return_by_q.values,
                                 ylim_percentiles[0]) * DECIMAL_TO_BPS)
        ymax = (nanpercentile(return_by_q.values,
                                 ylim_percentiles[1]) * DECIMAL_TO_BPS)
    else:
        ymin = None
        ymax = None
...
def plot_mean_quantile_returns_spread_time_series(mean_returns_spread, std_err=None, bandwidth=1, ax=None):
    ...
    if isinstance(mean_returns_spread, DataFrame):
        if ax is None:
    ...
    ylim = nanpercentile(abs(mean_returns_spread_bps.values), 95)
...
def plot_monthly_ic_heatmap(mean_monthly_ic, ax=None):
    ...
    mean_monthly_ic.index = MultiIndex.from_arrays(
...
def plot_quantile_average_cumulative_return(avg_cumulative_returns, by_quantile=False, std_bar=False, title=None, ax=None):
    """
    ...
    Parameters
    ----------
    avg_cumulative_returns: DataFrame
    ...
    palette = [cm.coolwarm(i) for i in linspace(0, 1, quantiles)]
...
def plot_events_distribution(events, num_bars=50, ax=None):
    ...
    ----------
    events : Series
        A Series whose index contains at least 'date' level.
    ...
    grouper = Grouper(level='date', freq=group_interval)
