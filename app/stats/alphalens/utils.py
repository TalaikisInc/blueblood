...
from pandas import (MultiIndex, qcut, DataFrame, Series, concat, cut, DateOffset, DatetimeIndex,
    date_range, Timedelta, get_option, set_option, Timestamp)
from numpy import sqrt, busday_count, nan, float64
import re
...
def quantize_factor(factor_data,
    ...
    no_raise: bool, optional
        ...
        exception would have been thrown are set to NaN
    ...
    def quantile_calc(x, _quantiles, _bins, _zero_aware, _no_raise):
        try:
            if _quantiles is not None and _bins is None and not _zero_aware:
                ...
                pos_bins = cut(x[x >= 0], _bins // 2,
                                  labels=False) + _bins // 2 + 1
                neg_bins = cut(x[x < 0], _bins // 2,
                                  labels=False) + 1
                return concat([pos_bins, neg_bins]).sort_index()
        except Exception as e:
            if _no_raise:
                return Series(index=x.index)
            raise e
...
def infer_trading_calendar(factor_idx, prices_idx):
    ...
    for day, day_str in enumerate(days_of_the_week):
        ...
        all_weekdays = date_range(full_idx.min(), full_idx.max(),
...
def compute_forward_returns(factor, prices, periods=(1, 5, 10), filter_zscore=None):
    """
    ...
    Returns
    -------
    forward_returns : DataFrame - MultiIndex
    ...
    forward_returns = DataFrame(index=MultiIndex.from_product([factor_dateindex, prices.columns], names=['date', 'asset']))
    ...
    for period in sorted(periods):
        ...
        #
        entries_to_test = min(30, len(fwdret.index), len(prices.index) - period)
        days_diffs = []
        ...
        period_len -= Timedelta(days=delta_days)
...
def demean_forward_returns(factor_data, grouper=None):
    ...
    Returns
    -------
    adjusted_forward_returns : DataFrame - MultiIndex
...
def print_table(table, name=None, fmt=None):
    ...
    prev_option = get_option('display.float_format')
    if fmt is not None:
        set_option('display.float_format', lambda x: fmt.format(x))

    display(table)

    if fmt is not None:
        set_option('display.float_format', prev_option)


def get_clean_factor(factor, forward_returns, groupby=None, binning_by_group=False, quantiles=5, bins=None, groupby_labels=None, max_loss=0.35,  zero_aware=False):
    """
    ...
    groupby : Series - MultiIndex or dict
    ...
    Returns
    -------
    merged_data : DataFrame - MultiIndex
    ...
    merged_data = forward_returns.astype(float64).copy()
    merged_data['factor'] = factor.astype(float64)
    ...
    if groupby is not None:
        if isinstance(groupby, dict):
            ...
            ss = Series(groupby)
            groupby = Series(index=factor.index, data=ss[factor.index.get_level_values('asset')].values)
        ...
        if groupby_labels is not None:
            ...
            sn = Series(groupby_labels)
            groupby = Series(index=groupby.index,
    ...
    print("max_loss is %.1f%%!" % (max_loss * 100))

    return merged_data


def get_clean_factor_and_forward_returns(factor, prices, groupby=None, binning_by_group=False, quantiles=5, bins=None, periods=(1, 5, 10), filter_zscore=20, groupby_labels=None, max_loss=0, zero_aware=False):
    """
    ...
    forward_returns = compute_forward_returns(factor, prices, periods, filter_zscore)

    factor_data = get_clean_factor(factor, forward_returns, groupby=groupby, groupby_labels=groupby_labels, \
        quantiles=quantiles, bins=bins, binning_by_group=binning_by_group, max_loss=max_loss, zero_aware=zero_aware)
...
def rate_of_return(period_ret, base_period):
    ...
    conversion_factor = (Timedelta(base_period) /
                         Timedelta(period_len))
...
def std_conversion(period_std, base_period):
    ...
    conversion_factor = (Timedelta(period_len) /
                         Timedelta(base_period))
    return period_std / sqrt(conversion_factor)
...
def add_custom_calendar_timedelta(input, timedelta, freq):
    ...
    Parameters
    ----------
    input : DatetimeIndex or Timestamp
    timedelta : Timedelta
    freq : DateOffset, optional

    Returns
    -------
    DatetimeIndex or Timestamp
        input + timedelta
    """
    days = timedelta.components.days
    offset = timedelta - Timedelta(days=days)
    return input + freq * days + offset


def diff_custom_calendar_timedeltas(start, end, freq):
    """
    Compute the difference between two Timedelta taking into consideration
    ...
    if weekmask is not None and holidays is not None:
        ...
        actual_days = busday_count(start, end, weekmask, holidays)
    else:
        # default, it is slow
        actual_days = date_range(start, end, freq=freq).shape[0] - 1
        if not freq.onOffset(start):
            actual_days -= 1

    timediff = end - start
    delta_days = timediff.components.days - actual_days
    return timediff - Timedelta(days=delta_days)
