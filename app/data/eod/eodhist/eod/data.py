...
from ._utils import (_init_session, _format_date, _sanitize_dates, _url, RemoteDataError)
...
def get_eod_data(symbol, exchange, start=None, end=None,
    ...
    if r.status_code == requests.codes.ok:
        df = read_csv(StringIO(r.text), skipfooter=1, parse_dates=[0], index_col=0)
        return df
    else:
...
def get_exchange_symbols(exchange_code,
    ...
    r = session.get(url, params=params)
    if r.status_code == requests.codes.ok:
        df = read_csv(StringIO(r.text), skipfooter=1, index_col=0)
        return df
    else:
        params["api_token"] = get_api_key()
        raise RemoteDataError(r.status_code, r.reason, _url(url, params))
