...
DATAFORMATS = dict(
    btcsv=feeds.BacktraderCSVData,
    vchartcsv=feeds.VChartCSVData,
    vcfile=feeds.VChartFile,
    sierracsv=feeds.SierraChartCSVData,
    mt4csv=feeds.MT4CSVData,
    yahoocsv=feeds.YahooFinanceCSVData,
    yahoocsv_unreversed=feeds.YahooFinanceCSVData,
    yahoo=feeds.YahooFinanceData,
)

try:
    DATAFORMATS['vcdata'] = feeds.VCData
except AttributeError:
    pass  # no comtypes available

try:
    DATAFORMATS['ibdata'] = feeds.IBData,
except AttributeError:
    pass  # no ibpy available

try:
    DATAFORMATS['oandadata'] = feeds.OandaData,
except AttributeError:
...
def btrun(pargs=''):
    ...
    # get and add signals
    signals = getobjects(args.signals, Indicator, signals, issignal=True)
    for sig, kwargs, sigtype in signals:
        stype = getattr(signal, 'SIGNAL_' + sigtype.upper())
        cerebro.add_signal(stype, sig, **kwargs)

    # get and add strategies
    strategies = getobjects(args.strategies, Strategy, strategies)
    for strat, kwargs in strategies:
        cerebro.addstrategy(strat, **kwargs)

    inds = getobjects(args.indicators, Indicator, indicators)
    for ind, kwargs in inds:
        cerebro.addindicator(ind, **kwargs)

    obs = getobjects(args.observers, Observer, observers)
    for ob, kwargs in obs:
        cerebro.addobserver(ob, **kwargs)

    ans = getobjects(args.analyzers, Analyzer, analyzers)
