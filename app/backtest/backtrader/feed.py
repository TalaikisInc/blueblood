...
from app.backtest.backtrader import *
from app.backtest.backtrader import (date2num, num2date, time2num, TimeFrame, dataseries,
                        metabase)

from app.backtest.backtrader.utils.py3 import with_metaclass, zip, range, string_types
from app.backtest.backtrader.utils import tzparse
    ...
    def dopostinit(cls, _obj, *args, **kwargs):
        ...
        if isinstance(_obj.p.fromdate, date):
            ...
            if not hasattr(_obj.p.fromdate, 'hour'):
                _obj.p.fromdate = datetime.combine(
                    _obj.p.fromdate, _obj.p.sessionstart)

        if isinstance(_obj.p.todate, date):
            # push it to the end of the day, or else intraday
            # values before the end of the day would be gone
            if not hasattr(_obj.p.todate, 'hour'):
                _obj.p.todate = datetime.combine(
                    _obj.p.todate, _obj.p.sessionend)

        _obj._barstack = collections.deque()  # for filter operationsF
...
class AbstractDataBase(with_metaclass(MetaAbstractDataBase,
    ...
    def _start_finish(self):
        ...
        self._tzinput = utils.date.Localizer(self._gettzinput())
    ...
    def _getnexteos(self):
        ...
        if self._calendar is None:
            nexteos = datetime.combine(dtime, self.p.sessionend)
            nextdteos = self.date2num(nexteos)  # locl'ed -> utc-like
            nexteos = num2date(nextdteos)  # utc
            while dtime > nexteos:
                nexteos += timedelta(days=1)  # already utc-like
